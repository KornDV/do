...
import sys
import getpass
print('\nHello! Let`s make server for Django project!\n')
project_name=input('Project name: ')
user_name=getpass.getuser()
print('\n--------------------------------------------------')
print('now instal: '+str(project_name)+'\nfor: '+str(user_name))
print('--------------------------------------------------\n')
print('All OK?')
q=input('Yes/No: ')
if q not in ['Yes','YES','yes','Y','y']:
	exit()

now_command='sudo mkdir /var/site; sudo apt-get update; sudo apt-get install -y python3-pip python3-venv nginx'
os.system(now_command)

now_command='sudo python3 -m venv /var/site/env'
os.system(now_command)

script_1='#!/bin/bash\nsource /var/site/env/bin/activate\npip3 install Django\ncd /var/site\ndjango-admin startproject '+str(project_name)+'\npip3 install gunicorn'
...
start_gunicorn='#!/bin/bash\nsource /var/site/env/bin/activate\nexec gunicorn -c "/var/site/'+str(project_name)+'/gunicorn_config.py" '+str(project_name)+'.wsgi'
